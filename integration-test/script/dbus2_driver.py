...
def client_cluster_wait(option):
  timeout=10;
  interval = 5
  regIds = get_child_registrations("localhost", options.client_base_port_list, options.client_cluster_name)

  url_template = "http://%s:%s/containerStats/inbound/events/total"
  maxWinScn = get_relay_inbound_total_maxStreamWinScn(url_template, options.relay_host, options.relay_port)

  print("Max Seen SCN in relay : %s" %(maxWinScn));

  ret = 0; 
  for regId in regIds:
    dbg_print ("Registration Id :" + regId)
    ret = wait_for_condition('consumer_cluster_reach_maxStreamWinScn(%s, "%s", %s, "%s", "%s")' % (maxWinScn, consumer_host, options.client_base_port_list, regId, option), timeout, interval)
    if ret == RetCode.TIMEOUT: 
       print ("Timed out waiting for client %s to reach SCN:%s for regId:%s" % ( options.client_base_port_list, maxWinScn, regId))
...
def verifyRegistrations():
    ...
    Url="http://localhost:" + options.client_base_port_list + "/clientState/registrations"
    dbg_print(Url)
...
def getV2ClientClusterRegistrations():
    regIds = get_child_registrations("localhost", options.client_base_port_list, options.client_cluster_name)
    for regId in regIds:
       print regId
...
cmd_dict_mp_beta={
    ...
    ,"profile_relay_debug":{"start":"databus2-linkedin-relay/databus2-linkedin-relay-run/run.py --Dtest=run --Denable_jvm_debug","stop": kill_cmd_template % "Member2RelayServer","stats":[get_stats,"Member2RelayServer"]}
    ...
    ,"multitenant_relay_deploy":{"start":[conf_and_deploy_GLU, "databus2-relay-war", "deploy"],"stop":[conf_and_deploy_GLU, "databus2-relay-war","undeploy"]}
    ...
    ,"liar_lb_consumer":{"start":"databus-profile-client/databus-profile-client-run/run.py --Dtest=run-liar-lb","stop": [stop_gracefully, "LiarLoadBalancedClient"],"get_child_regs":[getV2ClientClusterRegistrations],"wait_event":[client_cluster_wait, "stream"],"wait_event_bootstrap":[client_cluster_wait, "bootstrap"],"maxscn":[get_inbound_total_maxStreamWinScn],"pause":[pause_resume_consumer,"pause"],"resume":[pause_resume_consumer, "resume"], "shutdown":[shutdown],"shutdown_force":[shutdown,"force"]}
...
def run_cmd_add_config(cmd, check_mp_alpha=True):
    if options.operation in ["start","invoke", "clean_log"]: 
      ...
      if options.cluster_name: cmd = run_cmd_add_option(cmd, "cluster.name", options.cluster_name, check_mp_alpha)
...
def get_ebuf_inbound_total_maxStreamWinScn_client_cluster(host, port, regId=None, option=None, maxScnExtractor=None):
    url_template = ""
    if option == "bootstrap":
       url_template = "http://%s:%s/clientStats/bootstrap/events/registration/" + regId
    else:
       url_template = "http://%s:%s/clientStats/inbound/events/registration/" + regId
...
def consumer_cluster_reach_maxStreamWinScn(maxWinScn, host, port, regId=None, option=None, maxScnExtractor=None):
    consumerMaxWinScn = get_ebuf_inbound_total_maxStreamWinScn_client_cluster(host, port, regId, option, maxScnExtractor)
    print("consumerMaxWinScn = %s, maxWinScn = %s" % (consumerMaxWinScn, maxWinScn))
    return long(consumerMaxWinScn) >= long(maxWinScn)
...
def main(argv):
    ...
    config_group.add_option("--client_cluster_name", action="store", dest="client_cluster_name", default=None,
                       help="client cluster name")
