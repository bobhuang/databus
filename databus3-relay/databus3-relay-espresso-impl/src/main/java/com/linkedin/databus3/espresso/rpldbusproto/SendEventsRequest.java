...
//      if (debugEnabled) LOG.debug("sendEventsRequest: " + _cmd);
  ...
  public static class ExecHandler extends SimpleChannelHandler
  {
    ...
    private final LogMessageAccumulator _debugLogMsg;
                       ...
                       SchemaRegistryService schemaRegService, HttpRelay relay)
    {
       ...
       _debugLogMsg = new LogMessageAccumulator();
    }
    ...
    public void messageReceived(ChannelHandlerContext ctx, MessageEvent e) throws Exception
    {
      ...
      else if (e.getMessage() instanceof SendEventsRequest)
      {
        ...
        _debugLogMsg.append(",processTimeMs=" + msecs);
        if (_log.isDebugEnabled())
        {
          ...
          _log.debug(_debugLogMsg.toString());
        }
      }
      else
      {
                                        ...
                                        StartSendEventsRequest req) throws Exception
    {
      ...
      else if (3 == req.getProtocolVersion())
      {
        ...
        _eventRewriter = new EventRewriter(_curInSrcMap, _srcIdMap, _schemaRegService, _relay, req.getServerId(), _debugLogMsg);
      }
      else
      {
...
//                                      " for server " + _curServerName);
        ...
        {
          _log.error("unable to store binlog offset: " + e.getClass() + ":" + e.getMessage() + ":" + _debugLogMsg, e);
        }
      ...
      {
        _log.error("unable to find or create binlog offset writer:" + _debugLogMsg);
      }
    }
...
//                                                : Integer.MIN_VALUE);
    ...
    private void appendEventBytes(ChannelBuffer channelBuffer)
    {
      ...
      _debugLogMsg.append(ChannelBuffers.hexDump(channelBuffer));
...
//      if (debugEnabled) _log.debug("push: " + req.toJsonString(false));
      ...
      try
      {
    	  ...
    	  {
          _log.error("Rejecting data request without startSendEvents:" + _debugLogMsg);
    	  ...
    	  else if (1 == req.getProtocolVersion() || 3 == req.getProtocolVersion())
    	  {
    		  ...
    		  if(!DbusEvent.isControlSrcId(lSourceId.shortValue())) { // normal fake source id (negatives are for control)
    			  ...
    			  if (null != _eventRewriter)
    			  {
    				  ...
    				  _debugLogMsg.append(",chBufCopyMs=" + msecs);
    				  if (_perfLog.isDebugEnabled())
    				  ...
    				  try
    				  {
    					  ...
    					  _debugLogMsg.append(",rewriteMs=" + msecs);
    				  } catch (SchemaUpdateFailed su)
    				  {
    					  _log.error("Schema update happened, hence initiating a new connection from RPL_DBUS:" + _debugLogMsg, su);
    			  ...
    			  if (null == _mostRecentBuffer) {
    				  _log.error("Seems like control event occured before the actual data. We don't allow it:" + _debugLogMsg);
    		  ...
    		  if(! isControlEvent) {
    		    ...
    		    if(buf == null || _mostRecentSeenBinlogOffset == -1) {
    		      ...
    		      _log.error("Failing because it is a first event ever for " + errMsg + ";dbg=" + _debugLogMsg);
    		  ...
    		  if (null == result)
    		  {
    			  ...
    			  eventsNum = buf.readEvents(
    					  ...
    					  stats);
    			  _debugLogMsg.append(",nEvents=" + eventsNum);
    			  _debugLogMsg.append(",buf=" + buf.hashCode());
    	  ...
    	  {
    		  _log.error("Rejecting data request due to unsupported protocol version:" + _debugLogMsg);
    		  result  = ErrorResponse.createUnsupportedProtocolVersionResponse(req.getProtocolVersion());
    		  _debugLogMsg.append(",result=" + result.getClass().getName());
    	  }
      } catch (Exception e)
      {
        if (event != null)
        {
          _log.error("Exception on event:" + _debugLogMsg, e);
        }
    ...
    throws SendEventsRequestException {
      ...
      if(map != null) {
        ...
        _debugLogMsg.append(",lSource=" + lSource);

        if(lSource == null) {
          _debugLogMsg.append(",InvalidLogicalSrcMapping=" + lSourceId.toString());
        ...
        try {
          ...
          _debugLogMsg.append(",InvalidLogicalSrc=" + lSourceId.toString());
          throw new SendEventsRequestException(
                                               ErrorResponse.createInvalidRequestParam("sendEvents",
  ...
  static class EventRewriter extends InternalDatabusEventsListenerAbstract
  {
    ...
    public void onEvent(DataChangeEvent event, long offset, int size)
    {
      ...
      if (null != src) /* source present in SourceIdNameRegistry */
      {
        ...
        if (needSchemaUpdate)
        {
          _debugLogMsg.append(",schemaUpdate=true,srcName=" + sourceName);
          ...
          if (null == _outputSrcidMap.getSource(sourceName))
            LOG.error("outputSrcidMap not updated:" + _debugLogMsg);
          if (_relay != null && !_relay.getSourceSchemaVersionMap().containsKey(sourceName))
          {
            LOG.error("relay outputSrcidMap not updated:" + _debugLogMsg);
          }
        }
        LogicalSource dest = _outputSrcidMap.getSource(sourceName); /* obtain dest from SchemasRegistry */
        if (null == dest)
        {
          LOG.error("Source for the event not found in schemas registry:" + _debugLogMsg);
          return;
        }
        _debugLogMsg.append(",outSrc=" + dest.getId().shortValue());

        if (_relay != null && ! _relay.getSourceSchemaVersionMap().containsKey(sourceName))
        {
          LOG.info("This is the first time we see event from the source " + _debugLogMsg);
          _relay.getSourceSchemaVersionMap().put(sourceName, schemaVersion);
        }

        if(_schemaRegService == null)
        {
        	LOG.error("schemaRegistry is null:" + _debugLogMsg);
          return;
        }
        ...
        if (schemaId == null)
        {
        	LOG.error("schemaId is null for dest" + _debugLogMsg);
          return;
        }

        _debugLogMsg.append(",schemaId=" + schemaId.toString());
      ...
      {
        LOG.error("input source not found: " + _debugLogMsg);
      }
    }
    ...
    throws DatabusException
    {
      ...
      while(++count < NUM_RETRIES)
      {
    	  ...
    	  {
    		  LOG.info("Exception while trying to fetchSchemaIdForSourceNameAndVersion:" + _debugLogMsg, de);
    	  }
    	  try
    	  {
    	    Thread.sleep(100);
      ...
      LOG.error("waitForSchemaVersionUpdate failed to fetch schema version:" + _debugLogMsg);
      return false;
    }
    ...
    private void performSchemaUpdate(String sourceName, short schemaVersion)
    {
        ...
        {
          success = false;
          LOG.error("schema id not found:" +  _debugLogMsg, de);
        }
