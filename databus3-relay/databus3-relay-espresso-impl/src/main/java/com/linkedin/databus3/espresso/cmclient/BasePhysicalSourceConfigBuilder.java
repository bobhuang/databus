...
public abstract class BasePhysicalSourceConfigBuilder {
	...
	throws InvalidConfigException {
		...
		try {
			if (null != _sourceIdNameRegistry) {
				...
				if (null != id) {
					...
					final int SLEEP_BETWEEN_RETRIES_IN_MS = 100;
					for (int count=0;count < NUM_RETRIES;count++)
					{
						...
						{
							LOG.error("Could not obtain id for " + es.getDatabusSourceName() + " count = " + count);
							Thread.sleep(SLEEP_BETWEEN_RETRIES_IN_MS);
						}
					}
		...
		{
			throw new InvalidConfigException("Got an interrupted exception after failing to get an id for logical source");
		} catch (DatabusException e)
		{
			throw new InvalidConfigException(e);
		}
